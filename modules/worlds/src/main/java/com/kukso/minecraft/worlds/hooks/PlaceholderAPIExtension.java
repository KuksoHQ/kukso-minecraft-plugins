...
import java.util.logging.Level;
    ...
    /**
     * PlaceholderAPI extension for KuksoWorlds plugin.
     * This class registers placeholders that can be used by other plugins
     * (like scoreboard, chat, tablist, etc.) to show information from this plugin.
     */
public class PlaceholderAPIExtension extends PlaceholderExpansion {
    ...
    /**
     * Check if the player's current world is loaded by KuksoWorlds.
     *
     * @param player The player
    ...
    private String getWorldLoadedStatus(OfflinePlayer player) {
        ...
        // Check if world is managed by KuksoWorlds
     ...
     * This uses the KuksoWorlds custom spawn if available, otherwise falls back to Bukkit's spawn.
     *
    ...
    private String getWorldSpawnLocation(String worldName) {
        ...
        // Try to get custom spawn from KuksoWorlds configuration
        Location customSpawn = plugin.getWorldLoader().getSpawn(worldName);

        if (customSpawn != null) {
            debug("world_spawn: Using KuksoWorlds custom spawn for " + worldName);
    ...
    /**
     * Get the count of loaded KuksoWorlds.
     *
