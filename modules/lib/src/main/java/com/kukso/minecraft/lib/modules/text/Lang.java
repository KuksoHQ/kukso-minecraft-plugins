...
import com.kukso.minecraft.lib.utilities.ColorManager;
...
import com.kukso.minecraft.lib.utilities.PlaceholderManager;
...
public class Lang {
    ...
    public static void loadAdditional(Plugin plugin) {
        LocaleLoader.loadTranslations(plugin, translations, true);
    }
    ...
    public static void testNestedValue() {
        ...
        String value = getNestedValue(test, "kuksolib.status");
        LoggingManager logger = Bukkit.getServicesManager().load(LoggingManager.class);
        logger.log("[DEBUG] test kuksolib.status = " + value);
    }
}
