// Java
package com.kukso.minecraft.dialogs.utilities;
...
public final class CommandPlaceholderUtility {
    ...
    private static boolean hasPapi() {
        ...
        // Bukkit.getLogger().info("[KuksoDialogs][PAPI] Detected: " + available);
        return available;
    }
    ...
    private static String applyPapi(String input, Player player) {
        try {
            ...
            // Bukkit.getLogger().info("[KuksoDialogs][PAPI] Before: " + before);
            // Bukkit.getLogger().info("[KuksoDialogs][PAPI] After:  " + after);
            return after;
        } catch (Throwable t) {
            // Keep warnings for real failures
            // Bukkit.getLogger().warning("[KuksoDialogs][PAPI] Expansion failed: " + t.getClass().getSimpleName() + " - " + t.getMessage());
            return input;
        }
    }
    ...
    private static String applyBuiltinPapiFallbacks(String input, Player p) {
        ...
        //     Bukkit.getLogger().info("[KuksoDialogs][PAPI-Fallback] Applied built-in replacements:");
        //     Bukkit.getLogger().info("[KuksoDialogs][PAPI-Fallback] In:  " + before);
        //     Bukkit.getLogger().info("[KuksoDialogs][PAPI-Fallback] Out: " + out);
        // }
        return out;
    }
    ...
    public static String resolveCommandPlaceholders(String command,
        ...
        //     Bukkit.getLogger().info("[KuksoDialogs][Placeholders] Native replaced:");
        //     Bukkit.getLogger().info("[KuksoDialogs][Placeholders] In:  " + original);
        //     Bukkit.getLogger().info("[KuksoDialogs][Placeholders] Out: " + result);
        // }

        if (isPlayer && result.contains("%") && hasPapi()) {
            result = applyPapi(result, (Player) audience);
        } else if (result.contains("%")) {
            // Bukkit.getLogger().info("[KuksoDialogs][PAPI] Skipped (no PAPI or no player context). Command: " + result);
        }
