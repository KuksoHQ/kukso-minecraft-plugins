...
/**
 * Entry point for KuksoDialogs' public API.
 * Consumers call KuksoDialogsAPI.get() to obtain the registry for registering action listeners.
 *
...
public final class KuksoDialogsAPI {
    ...
    public static DialogActionRegistry get() {
        ...
        if (reg == null) {
            throw new IllegalStateException("KuksoDialogs API is not available yet. Is the plugin enabled?");
        }
        return reg;
    }
    ...
    public static void bootstrap(DialogActionRegistry registry, String apiVersion) {
        ...
        synchronized (KuksoDialogsAPI.class) {
            if (REGISTRY != null) {
                throw new IllegalStateException("KuksoDialogs API is already initialized");
            }
            REGISTRY = registry;
