package com.kukso.minecraft.dialogs.API.addon;
...
/**
 * Manages the discovery, loading, and lifecycle of KuksoDialogs addons.
 */
public class AddonManager {
    ...
    private void loadAddon(File jarFile) throws Exception {
        ...
        // Use ServiceLoader to find KuksoDialogsAddon implementations
        ServiceLoader<KuksoDialogsAddon> serviceLoader = ServiceLoader.load(
                KuksoDialogsAddon.class, 
                classLoader
        );
        
        List<KuksoDialogsAddon> discoveredAddons = new ArrayList<>();
        for (KuksoDialogsAddon addon : serviceLoader) {
            discoveredAddons.add(addon);
        }
        
        if (discoveredAddons.isEmpty()) {
            logger.warning("No KuksoDialogsAddon implementation found in " + jarFile.getName());
            classLoader.close();
            return;
        }
        
        if (discoveredAddons.size() > 1) {
            logger.warning("Multiple KuksoDialogsAddon implementations found in " + 
                    jarFile.getName() + ". Using the first one.");
        }
        
        KuksoDialogsAddon addonInstance = discoveredAddons.get(0);
        ...
        AddonContext context = new com.kukso.minecraft.dialogs.API.addon.SimpleAddonContext(
                plugin,
                addonId,
                apiVersion
        );
    ...
    private static class LoadedAddon {
        final String id;
        final KuksoDialogsAddon instance;
        final URLClassLoader classLoader;
        final String jarPath;
        
        LoadedAddon(String id, KuksoDialogsAddon instance, URLClassLoader classLoader, String jarPath) {
            this.id = id;
